...
<body>
    ...
    @foreach ($posts as $post)
        <li>{{ $post['user_id'] }} --> {{ $post['username'] }} --> {{ $post['password'] }} 
            <form action="{{ route('data-akun.destroy', $post['user_id']) }}" method="POST">
                @csrf
                @method('DELETE')
    ...
    @foreach ($akun as $post)
        <li>{{ $post->user_id }} --> {{ $post->sername }} --> {{ $post->password }} 
            <form action="{{ route('data-akun.destroy', $post['user_id']) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit">Delete</button>
    ...
    @foreach ($akuns as $akun)
        <li>{{ $akun['nis'] }} --> {{ $akun['user_id'] }} --{{ $akun['id_kelas'] }} -->  {{ $akun['nama'] }} 
            <form action="{{ route('data-siswa.destroy', $akun['nis']) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit">Delete</button>
            </form>
